...
| **요금이 몇 배로 뜀** | **프롬프트 캐시 앞에 매번 바뀌는 내용을 두면** 캐시가 통째로 무효가 된다. 설교 원고 5,400토큰이 문장마다 1.25배로 새로 청구되어 한 시간 예배에 $40이 나온 적 있음. 변하는 것(문맥·시각·ID)은 **반드시 `cache_control` 블록 뒤에** 둘 것. 확인법은 `로그.txt`의 `캐시읽기`가 0인지 보는 것 |
